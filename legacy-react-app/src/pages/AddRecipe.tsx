...
interface AddRecipeForm {
    ...
    imageFile?: FileList; // New image file
    cookTime: string;
    servings: number;
    category: string;
...
export const AddRecipe: React.FC = () => {
    ...
    const onSubmit = async (data: AddRecipeForm) => {
        try {
            ...
            // Note: If no new file, backend keeps existing image for update. 
            // If creation and no file, it's just undefined.
            ...
            navigate(isEditing ? `/recipes/${id}` : '/recipes');
        } catch (err) {
    ...
    return (
        <div className="max-w-3xl mx-auto space-y-8 pb-12 animate-fade-in">
            <div className="flex items-center justify-between">
                ...
                <Button
                    variant="ghost"
                    onClick={() => navigate(isEditing ? `/recipes/${id}` : '/dashboard')}
                >
